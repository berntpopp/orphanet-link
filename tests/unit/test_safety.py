...
def test_injection_survives_real_tool_round_trip() -> None:
    """The DATA-plane service returns embedded instruction text verbatim as data.

    ``OrphanetService`` is the data plane: it passes free-text through unchanged
    (a bare string), never acting on it. The Response-Envelope v1.1
    ``untrusted_text`` reshaping happens one layer up, at the MCP tool boundary
    (proven by tests/unit/mcp/test_untrusted_content_fencing.py driving the real
    FastMCP tool). Uses a fresh ``OrphanetService`` with an injected stub repo, so
    the global service singleton the session ``facade`` depends on is never touched.
    """
