...
format: ## Format Python code
	uv run ruff format orphanet_link tests server.py mcp_server.py scripts

format-check: ## Check formatting without writing
	uv run ruff format --check orphanet_link tests server.py mcp_server.py scripts

lint: ## Lint Python code
	uv run ruff check orphanet_link tests server.py mcp_server.py scripts

lint-ci: ## Lint with GitHub-Actions output
	uv run ruff check orphanet_link tests server.py mcp_server.py scripts --output-format=github

lint-fix: ## Lint and apply safe fixes
	uv run ruff check orphanet_link tests server.py mcp_server.py scripts --fix
...
test-cov: ## Run tests with coverage
	uv run pytest tests -m "not integration" --cov=orphanet_link --cov-report=term-missing --cov-report=html
...
data: ## Download Orphanet and build the local index
...
docker-url: ## Print the MCP URL (host port the container is published on)
	@hostport=$$($(COMPOSE) port orphanet-link 8000 2>/dev/null); \
	port=$${hostport##*:}; \
	if [ -n "$$port" ]; then \
	  echo "orphanet-link MCP: http://127.0.0.1:$$port/mcp  (health: http://127.0.0.1:$$port/health)"; \
	  echo "Claude Code: claude mcp add --transport http orphanet-link --scope user http://127.0.0.1:$$port/mcp"; \
	else \
	  echo "orphanet-link container is not running. Start it with: make docker-up"; \
	fi

info: ## Show project information
	@echo "Project: orphanet-link"
